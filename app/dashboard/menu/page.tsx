...
import { uploadMenuItemImage, deleteMenuItemImage, saveMenu, subscribeMenu } from '@/lib/supabase'
...
const DEFAULT_MENU: Category[] = [
  ...
  { id: 'c2', name: 'Основні', emoji: '🍽', items: [
...
export default function MenuPage() {
  ...
  useEffect(() => {
    const unsub = subscribeMenu(RESTAURANT_ID, (menu) => {
      if (menu && menu.length > 0) {
        setCategories(menu as Category[])
        setActiveCategory(prev => menu.some(c => c.id === prev) ? prev : menu[0].id)
      }
      setLoading(false)
    })
    return () => unsub()
  ...
  const saveItem = () => {
    ...
    setCategories(cats => cats.map(c => ({ ...c, items: c.items.map(i => i.id === editingItem.id ? editingItem : i) })))
    setEditingItem(null)
  }

  const addItem = () => {
    if (!newItem.name || !newItem.price) return
    const item: MenuItem = { id: 'i' + Date.now(), name: newItem.name, desc: newItem.desc, price: Number(newItem.price), available: true, imageUrl: newItem.imageUrl || undefined }
    setCategories(cats => cats.map(c => c.id === activeCategory ? {...c, items: [...c.items, item]} : c))
  ...
  const deleteItem = (catId: string, itemId: string) => {
    ...
    setCategories(cats => cats.map(c => c.id === catId ? {...c, items: c.items.filter(i => i.id !== itemId)} : c))
  }

  const toggleAvailable = (catId: string, itemId: string) => {
    setCategories(cats => cats.map(c => c.id === catId ? {...c, items: c.items.map(i => i.id === itemId ? {...i, available: !i.available} : i)} : c))
  }
  ...
  const handleNewImageSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    } catch { alert('Не вдалося завантажити фото.') } finally { setUploadingNew(false) }
  }
  ...
  const handleEditImageSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    } catch { alert('Не вдалося завантажити фото.') } finally { setUploadingEdit(false) }
  }

  if (loading) return <div className="flex items-center justify-center min-h-screen"><div className="text-[#9A9490] text-sm">Завантаження меню...</div></div>
          ...
          <div className="bg-white border border-[#E8E0D4] rounded-2xl p-3 space-y-1">
            {categories.map(cat => (
              <button key={cat.id} onClick={() => setActiveCategory(cat.id)}
                className={`w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors text-left ${activeCategory === cat.id ? 'bg-[#C17F3B] text-white' : 'text-[#1C1A18] hover:bg-[#FAF8F5]'}`}>
                <span>{cat.emoji}</span>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => setShowAddItem(true)}
              className="flex items-center gap-1.5 px-4 py-2 bg-[#C17F3B] text-white rounded-xl text-sm font-semibold hover:bg-[#9A6328]">
              + Додати страву
            </button>
          </div>
          ...
          {activeCat.items.length === 0 ? (
            <div className="bg-white border-2 border-dashed border-[#E8E0D4] rounded-2xl p-12 text-center">
              ...
              <button onClick={() => setShowAddItem(true)} className="px-6 py-2.5 bg-[#C17F3B] text-white rounded-xl text-sm font-semibold">Додати першу страву</button>
            </div>
          ) : (
            <div className="space-y-2">
              {activeCat.items.map(item => (
                <div key={item.id} className={`bg-white border rounded-2xl p-4 flex items-center gap-4 ${item.available ? 'border-[#E8E0D4]' : 'border-[#E8E0D4] opacity-60'}`}>
                  {item.imageUrl ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      ...
                      {!item.available && <span className="text-xs bg-red-100 text-red-600 px-2 py-0.5 rounded-full">Недоступно</span>}
                    </div>
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <button onClick={() => toggleAvailable(activeCat.id, item.id)}
                      className={`w-8 h-8 rounded-lg text-sm ${item.available ? 'bg-[#E6F4ED] text-[#3A7D58]' : 'bg-gray-100 text-gray-400'}`}>
                      {item.available ? '✓' : '✗'}
                    </button>
                    <button onClick={() => setEditingItem(item)} className="w-8 h-8 rounded-lg bg-[#F5E9D8] text-[#C17F3B] text-sm hover:bg-[#EDD5B3]">✏</button>
                    <button onClick={() => deleteItem(activeCat.id, item.id)} className="w-8 h-8 rounded-lg bg-[#FDECEA] text-red-500 text-sm hover:bg-[#FACBC8]">✕</button>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                {newItem.imageUrl ? (
                  <div className="relative w-full h-36 rounded-xl overflow-hidden border border-[#E8E0D4]">
                    ...
                    <button onClick={() => setNewItem(p => ({...p, imageUrl: ''}))} className="absolute top-2 right-2 w-7 h-7 rounded-full bg-black/50 text-white text-sm flex items-center justify-center">✕</button>
                  </div>
                ) : (
                  <button onClick={() => newImageInputRef.current?.click()} disabled={uploadingNew}
                    className="w-full h-36 border-2 border-dashed border-[#E8E0D4] rounded-xl flex flex-col items-center justify-center gap-2 text-[#9A9490] hover:border-[#C17F3B] hover:text-[#C17F3B] disabled:opacity-60">
              ...
              <div>
                ...
                <input value={newItem.name} onChange={e => setNewItem({...newItem, name: e.target.value})}
                  className="w-full px-4 py-3 border border-[#E8E0D4] rounded-xl focus:border-[#C17F3B] focus:outline-none text-sm" placeholder="Борщ з пампушками" />
              </div>
              <div>
                ...
                <input value={newItem.desc} onChange={e => setNewItem({...newItem, desc: e.target.value})}
                  className="w-full px-4 py-3 border border-[#E8E0D4] rounded-xl focus:border-[#C17F3B] focus:outline-none text-sm" placeholder="Короткий опис інгредієнтів" />
              </div>
              <div>
                ...
                <input type="number" value={newItem.price} onChange={e => setNewItem({...newItem, price: e.target.value})}
                  className="w-full px-4 py-3 border border-[#E8E0D4] rounded-xl focus:border-[#C17F3B] focus:outline-none text-sm" placeholder="149" />
              </div>
              <div className="flex gap-3 pt-2">
                ...
                <button onClick={addItem} className="flex-1 py-3 bg-[#C17F3B] text-white rounded-xl text-sm font-semibold">Додати</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-end sm:items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl">
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                <button onClick={() => setEditingItem(null)} className="flex-1 py-3 border border-[#E8E0D4] rounded-xl text-sm font-medium">Скасувати</button>
                <button onClick={saveItem} className="flex-1 py-3 bg-[#C17F3B] text-white rounded-xl text-sm font-semibold">Зберегти</button>
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={newCat.name} onChange={e => setNewCat({...newCat, name: e.target.value})}
                  className="w-full px-4 py-3 border border-[#E8E0D4] rounded-xl focus:border-[#C17F3B] focus:outline-none text-sm" placeholder="Десерти" />
              </div>
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {EMOJIS.map(e => (
                    <button key={e} onClick={() => setNewCat({...newCat, emoji: e})}
                      className={`w-10 h-10 rounded-xl text-xl transition-all ${newCat.emoji === e ? 'bg-[#C17F3B] scale-110' : 'bg-[#FAF8F5] hover:bg-[#F0E8DC]'}`}>{e}</button>
                  ))}
                </div>
              </div>
              <div className="flex gap-3 pt-2">
                <button onClick={() => setShowAddCat(false)} className="flex-1 py-3 border border-[#E8E0D4] rounded-xl text-sm font-medium">Скасувати</button>
                <button onClick={addCategory} className="flex-1 py-3 bg-[#C17F3B] text-white rounded-xl text-sm font-semibold">Додати</button>
              </div>
            </div>
          </div>
        </div>
